...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
        # PyTorch 2.6 changed torch.load default weights_only=True, which breaks YOLOv7 checkpoints
        # that store full model objects (e.g., models.yolo.Model). We explicitly set weights_only=False
        # for backward compatibility. Do this only if you trust the checkpoint source.
        try:
